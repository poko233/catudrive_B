...
<body>
    ...
    <table class="summary" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <td>
                <span class="summary-label">Finalizadas / Inactivas</span>
                <span class="summary-value">{{ $reporte['resumen_estado']['Inactivo'] ?? 0 }}</span>
            </td>
            <td>
                <span class="summary-label">Estado filtrado</span>
                <span class="summary-value">{{ $filtros['estado_asignacion'] ?? 'Todos' }}</span>
            </td>
        </tr>
    </table>
                ...
                @foreach($reporte['items'] as $item)
                    ...
                    <tr>
                        ...
                        <td>
                            {{ $item->vehiculo?->placa ?? '-' }}
                            ·
                            {{ trim(($item->vehiculo?->marca ?? '') . ' ' . ($item->vehiculo?->modelo ?? '')) ?: '-' }}
                        </td>
                        <td>{{ $item->vehiculo?->tipo ?? '-' }}</td>
                        <td>{{ $item->fecha_asignacion?->format('d/m/Y') ?? '-' }}</td>
                        <td>{{ $item->fecha_finalizacion?->format('d/m/Y') ?? '-' }}</td>
                        <td>{{ $item->estado }}</td>
    ...
    @else
        <div class="empty">No existen asignaciones para los filtros seleccionados.</div>
    @endif
